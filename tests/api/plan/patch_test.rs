...
fn execute_patch_plan(cook_and_run_id: &Uuid, token: &str) -> reqwest::blocking::Response {
...
pub fn patch_plan(cook_and_run_id: &Uuid, token: &str) {
    let res = execute_patch_plan(cook_and_run_id, token);
